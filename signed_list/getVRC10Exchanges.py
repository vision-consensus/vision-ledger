...
for E in exchanges['exchanges']:
    ...
    token1 = {}
    token2 = {}
    if vision.toText(E['first_token_id'])=='_':
        token1 = {
            "name": "545258",
            "precision": 6
        }
    else:
        data = urllib.request.urlopen("{}/wallet/getassetissuebyid?value={}".format(full_node,vision.toText(E['first_token_id'])))
        token1 = json.loads(data.read().decode())
    
    if vision.toText(E['second_token_id'])=='_':
        token2 = {
            "name": "545258",
            "precision": 6
        }
    else:
        data = urllib.request.urlopen("{}/wallet/getassetissuebyid?value={}".format(full_node,vision.toText(E['second_token_id'])))
