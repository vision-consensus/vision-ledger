                ...
                switch(context->stage) {
                    ...
                    case 2:
                        ...
                        switch (content->contractType){
                            case TRANSFERCONTRACT: // Send VS
                                switch(field) {
                                    case 1: //owner_address
                                        if (type!=2) THROW(0x6a80);
                                        // set VS token name
                                        content->tokenNamesLength[0]=4;
                                        os_memmove(content->tokenNames[0],"VS\0",content->tokenNamesLength[0]);
                                        // get owner address
                            ...
                            switch(field) {
                                    ...
                                    case 4: //data
                                        ...
                                        // get selector
                                        PRINTF("Selector: %08x\n",U4BE(buffer, offset));
                                        if (os_memcmp(&buffer[offset], SELECTOR[0], 4) == 0) content->VRC20Method = 1; // check if transfer(address, uint256) function
                                        else if (os_memcmp(&buffer[offset], SELECTOR[1], 4) == 0) content->VRC20Method = 2; // check if approve(address, uint256) function
                                        else {
                                            if (!customContract) {
                                                // NOT ALLOWED
