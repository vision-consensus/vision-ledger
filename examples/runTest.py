...
tx.transaction.raw_data.data = b'CryptoChain-VisionSR Ledger Transactions Tests'
...
#######################################
# Send VRC10 Token with Name/Decimals #
#######################################
...
########################################
# VRC10 Exchange Create with Token Name#
########################################
...
c = tx.raw_data.contract.add()
c.type = vision.Transaction.Contract.ExchangeTransactionContract
param = Any()
param.Pack(newContract)
c.parameter.CopyFrom(param)
...
c = tx.raw_data.contract.add()
c.type = vision.Transaction.Contract.FreezeBalanceContract
param = Any()
param.Pack(newContract)
c.parameter.CopyFrom(param)
...
#################################
# Freeze Balance Delegate Entropy#
#################################
logger.debug('\n\nFreeze Contract delegate entropy:')
...
####################################
# Unfreeze Balance Delegate Entropy #
####################################
logger.debug('\n\nUnfreeze Contract delegate entropy:')
...
c = tx.raw_data.contract.add()
c.type = vision.Transaction.Contract.WithdrawBalanceContract
param = Any()
param.Pack(newContract)
c.parameter.CopyFrom(param)
...
c = tx.raw_data.contract.add()
c.type = vision.Transaction.Contract.ProposalCreateContract
param = Any()
param.Pack(newContract)
c.parameter.CopyFrom(param)
...
c = tx.raw_data.contract.add()
c.type = vision.Transaction.Contract.ProposalApproveContract
param = Any()
param.Pack(newContract)
c.parameter.CopyFrom(param)
...
c = tx.raw_data.contract.add()
c.type = vision.Transaction.Contract.ProposalDeleteContract
param = Any()
param.Pack(newContract)
c.parameter.CopyFrom(param)
...
c = tx.raw_data.contract.add()
c.type = vision.Transaction.Contract.AccountUpdateContract
param = Any()
param.Pack(newContract)
c.parameter.CopyFrom(param)
...
# VRC20 Approve  #
##################
logger.debug('\n\SmartContract Trigger VRC20 Approve Transfer:')
